...
void test_adding_simple_intergers() {
  ...
  assert_long_equal(3, sum.coefficient);
...
void test_adding_floats() {
  ...
  decimal sum = dec_add(a, b);
  assert_long_equal(2225, sum.coefficient);
...
void test_adding_with_overflow () {
  ...
  decimal sum = dec_add(a, b);
  assert_long_equal(18600000000000, sum.coefficient);
...
void test_adding_int_to_negative_float () {
  ...
  decimal b = {.coefficient = 2, .exponent = 0};
  decimal sum = dec_add(a, b);
  assert_long_equal(10000000000000, sum.coefficient);
  assert_int_equal(-13, sum.exponent);
}
...
void test_adding_int_to_float () {
  ...
  decimal sum = dec_add(a, b);
  assert_long_equal(14074000000000, sum.coefficient);
  assert_int_equal(-9, sum.exponent);
}
...
void test_adding_big_ints_with_overflow () {
  decimal a = {.coefficient = 14073748835533, .exponent = 2};
  decimal b = {.coefficient = 156374987061480, .exponent = 0};
  decimal sum = dec_add(a, b);
  assert_long_equal(15637498706147, sum.coefficient);
  assert_int_equal(2, sum.exponent);
}
...
void test_dividing () {
  ...
  assert_long_equal(9300000000000, r.coefficient);
  assert_int_equal(-13, r.exponent);
}

uint8_t qlog10(uint64_t v);
...
void dec_math_tests() {
  ...
  run_test(test_adding_big_ints_with_overflow);
